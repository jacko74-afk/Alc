...
import { SEED_IMAGES } from "./product-images";

const LOTTE =
  "https://m.kor.lottedfs.com/kr/display/category/second?dispShopNo1=10055924&dispShopNo2=10055930&treDpth=2";
const SHILLA = "https://m.shilladfs.com/estore/kr/ko/c/1211";
const SHINSEGAE = "https://www.ssgdfs.com/kr/dispctg/ctg/liquor/whisky";
...
type Row = [
  ...
  volumeMl: number,
  category: Category,
  lotte: [number | null, number | null, number | null],
  shilla: [number | null, number | null, number | null],
  shinsegae: [number | null, number | null, number | null],
];
...
const rows: Row[] = [
  ...
  ["kavalan-12-700", "카발란", "12년 더블캐스크", 700, "other", [72, 68, 110], [null, 70, 110], [74, 69, 110]],
  ...
  ["yamazaki-12-700", "야마자키", "12년", 700, "japanese", [null, 128, 180], [132, 132, 180], [135, 125, 180]],
  ...
  ["mars-iwai-700", "마르스", "이와이 트레디션", 700, "japanese", [34, 32, 45], [32.5, 32.5, 45], [null, 33, 45]],
...
export const allProducts: Product[] = rows.map((row) => {
  const [id, brand, name, volumeMl, category, lotte, shilla, shinsegae] = row;
  return {
    id,
    brand,
    name,
    ...
    isSamplePrice: true,
    imageUrl: SEED_IMAGES[id] ?? null,
    listings: {
      lotte: listing(lotte[0], lotte[1], lotte[2], LOTTE),
      shilla: listing(shilla[0], shilla[1], shilla[2], SHILLA),
      shinsegae: listing(shinsegae[0], shinsegae[1], shinsegae[2], SHINSEGAE),
    },
  };
});

export const DEFAULT_SEED_COUNT = 20;
export const SEED_COUNT_OPTIONS = [
  ...new Set([10, 20, 30, 40, allProducts.length].filter((n) => n <= allProducts.length)),
];

export function takeProducts(count: number, live: Product[] = []) {
  const n = Math.min(Math.max(1, count), allProducts.length);
  const byId = new Map(live.map((item) => [item.id, item]));
  return allProducts.slice(0, n).map((item) => byId.get(item.id) ?? item);
}

export const products = takeProducts(DEFAULT_SEED_COUNT);
